...
//play intro
introSound = new Audio("audio/tetrisstart.mp3");
...
function drawPausedScreen() {
  ...
  context.fillText(
    "Space to resume, Escape to exit",
    canvas.width / 2,
    canvas.height / 1.8
  );
}
...
function drawMainMenu() {
  ...
  context.font = "32px Russo One";
  context.textAlign = "center";
  context.fillText("Lets Play the game..", canvas.width / 2, canvas.height / 2);
  ...
  matrix.forEach((row, y) => {
    row.forEach((value, x) => {
      if (value !== 0) {
        ...
        context
          .roundRect(
            x * blockSize + offset.x * blockSize,
            100 + y * blockSize + offset.y * blockSize,
            1 * blockSize,
            1 * blockSize,
            5
          )
...
function playerRotate(dir) {
  ...
  if (player.spins > 25) {
    ...
    highspinsSound.addEventListener(
      "ended",
      function () {
